...
class AdvancedApi
{
    ...
    public function createRequest($environment, $path, $file = null, string $contentType = self::contentTypes['create'][0])
    {
        ...
        if (!preg_match("/.+/", $path)) {
            throw new \InvalidArgumentException("invalid value for \"path\" when calling AdvancedApi.create, must conform to the pattern /.+/.");
        }
    ...
    public function createOrUpdateRequest($environment, $path, $file = null, string $contentType = self::contentTypes['createOrUpdate'][0])
    {
        ...
        if (!preg_match("/.+/", $path)) {
            throw new \InvalidArgumentException("invalid value for \"path\" when calling AdvancedApi.createOrUpdate, must conform to the pattern /.+/.");
        }
    ...
    public function doAsyncRequest($file_extract_request, string $contentType = self::contentTypes['doAsync'][0])
    {
        ...
        if (isset($file_extract_request)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                ...
                $httpBody = json_encode(ObjectSerializer::sanitizeForSerialization($file_extract_request), JSON_THROW_ON_ERROR);
            } else {
     ...
     * @param  string $path  (required)
     * @param  \SplFileObject|null $file file (optional)
     * @param  string $contentType The value for the Content-Type header. Check self::contentTypes['doValidate'] to see the possible values for this operation
     *
     * @throws \HubSpot\Client\Cms\SourceCode\ApiException on non-2xx response or if the response body is not in the expected format
     * @throws \InvalidArgumentException
     * @return \HubSpot\Client\Cms\SourceCode\Model\Error
     */
    public function doValidate($environment, $path, $file = null, string $contentType = self::contentTypes['doValidate'][0])
    {
        list($response) = $this->doValidateWithHttpInfo($environment, $path, $file, $contentType);
        return $response;
    }
     ...
     */
    public function doValidateWithHttpInfo($environment, $path, $file = null, string $contentType = self::contentTypes['doValidate'][0])
    {
        $request = $this->doValidateRequest($environment, $path, $file, $contentType);
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function doValidateAsync($environment, $path, $file = null, string $contentType = self::contentTypes['doValidate'][0])
    {
        return $this->doValidateAsyncWithHttpInfo($environment, $path, $file, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function doValidateAsyncWithHttpInfo($environment, $path, $file = null, string $contentType = self::contentTypes['doValidate'][0])
    {
     ...
     * @return \GuzzleHttp\Psr7\Request
     */
    public function doValidateRequest($environment, $path, $file = null, string $contentType = self::contentTypes['doValidate'][0])
    {

        // verify the required parameter 'environment' is set
        if ($environment === null || (is_array($environment) && count($environment) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $environment when calling doValidate'
            );
        }
        if (!preg_match("/.+/", $environment)) {
            throw new \InvalidArgumentException("invalid value for \"environment\" when calling AdvancedApi.doValidate, must conform to the pattern /.+/.");
        }
        ...
        if (!preg_match("/.+/", $path)) {
            throw new \InvalidArgumentException("invalid value for \"path\" when calling AdvancedApi.doValidate, must conform to the pattern /.+/.");
        }
