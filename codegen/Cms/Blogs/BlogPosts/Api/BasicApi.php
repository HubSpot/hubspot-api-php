...
class BasicApi
{
    ...
    public function createRequest($blog_post, string $contentType = self::contentTypes['create'][0])
    {
        ...
        if (isset($blog_post)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                # if Content-Type contains "application/json", json_encode the body
                $httpBody = json_encode(ObjectSerializer::sanitizeForSerialization($blog_post), JSON_THROW_ON_ERROR);
            } else {
                $httpBody = $blog_post;
            }
        } elseif (count($formParams) > 0) {
     ...
     * @param  string|null $property Specific properties to return. (optional)
