...
class OwnersApi
{
     ...
     * @param  string|null $id_property id_property (optional, default to 'id')
            ...
            switch($statusCode) {
                case 200:
                    if ('\HubSpot\Client\Crm\Owners\Model\PublicOwner' === '\SplFileObject') {
                        ...
                        if ('\HubSpot\Client\Crm\Owners\Model\PublicOwner' !== 'string') {
                            try {
                ...
                default:
                    if ('\HubSpot\Client\Crm\Owners\Model\Error' === '\SplFileObject') {
                        ...
                        $content = (string) $response->getBody();
                        if ('\HubSpot\Client\Crm\Owners\Model\Error' !== 'string') {
                            try {
                                $content = json_decode($content, false, 512, JSON_THROW_ON_ERROR);
                            } catch (\JsonException $exception) {
                                throw new ApiException(
     ...
     *
     * @param  string|null $email Filter by email address (optional) (optional)
     ...
     * @param  int|null $limit The maximum number of results to display per page. (optional, default to 100)
     * @param  bool|null $archived Whether to return only results that have been archived. (optional, default to false)
            ...
            switch($statusCode) {
                case 200:
                    if ('\HubSpot\Client\Crm\Owners\Model\CollectionResponsePublicOwnerForwardPaging' === '\SplFileObject') {
                        ...
                        $content = (string) $response->getBody();
                        if ('\HubSpot\Client\Crm\Owners\Model\CollectionResponsePublicOwnerForwardPaging' !== 'string') {
                            try {
                                $content = json_decode($content, false, 512, JSON_THROW_ON_ERROR);
                            } catch (\JsonException $exception) {
                                throw new ApiException(
