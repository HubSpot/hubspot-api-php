...
class BatchApi
{
            ...
            switch($statusCode) {
                case 200:
                    return $this->handleResponseWithDataType(
                        '\HubSpot\Client\Crm\Lists\Model\BatchResponseRecordIdWithMemberships',
                        $request,
                        $response,
                    );
                case 207:
                    return $this->handleResponseWithDataType(
                        '\HubSpot\Client\Crm\Lists\Model\BatchResponseRecordIdWithMembershipsWithErrors',
                        $request,
                        $response,
                    );
                default:
    ...
    public function crmV3ListsRecordsMembershipsBatchReadRequest($batch_input_record_id_input, string $contentType = self::contentTypes['crmV3ListsRecordsMembershipsBatchRead'][0])
    {
        ...
        $resourcePath = '/crm/v3/lists/records/memberships/batch/read';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
        ...
        // for model (json/xml)
        if (isset($batch_input_record_id_input)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                # if Content-Type contains "application/json", json_encode the body
                $httpBody = json_encode(ObjectSerializer::sanitizeForSerialization($batch_input_record_id_input), JSON_THROW_ON_ERROR);
            } else {
                $httpBody = $batch_input_record_id_input;
            }
